    ...
    fileprivate func configureDataSource() {
        dataSource.configureCell = {
            ...
            if let strongSelf = self {
                cell.initCell(user: item, addAction: strongSelf.viewModel.onAddFriend(userID: item.id!), downloadAction: strongSelf.viewModel.downloadImage(url: baseURL.appendingPathComponent(item.profilePic!)))
            }
            return cell
        }
    }
